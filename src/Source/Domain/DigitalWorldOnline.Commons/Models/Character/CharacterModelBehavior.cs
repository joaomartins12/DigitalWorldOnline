    ...
    public sealed partial class CharacterModel
    {
        ...
        public int SocketAttribute(int baseValue, AccessoryStatusTypeEnum attribute)
        {
            ...
            foreach (var item in Equipment.EquippedItems)
            {
                ...
                {
                    //Console.WriteLine($"Socket Item Type: {accessoriesWithValueGreaterThanZero.Type}");

                    // Se for qualquer atributo diferente de AS
                    if (attribute == accessoriesWithValueGreaterThanZero.Type && attribute != AccessoryStatusTypeEnum.AS)
                    {
                        var applyAtt = (double)item.ItemInfo.ApplyElement / 100;

                        foreach (var accValue in item.AccessoryStatus)
                        {
                            if (accValue.ItemId == accessoriesWithValueGreaterThanZero.ItemId)
                            {
                    ...
                    // Se for AS (Attack Speed ou Move Speed)
                    else if (attribute == AccessoryStatusTypeEnum.AS && accessoriesWithValueGreaterThanZero.Type == AccessoryStatusTypeEnum.AS)
                    {
                        //Console.WriteLine($"AccessoryStatusTypeEnum.AS encontrado no Socket");

                        var applyAtt = (double)item.ItemInfo.ApplyElement / 100;

                        foreach (var accValue in item.AccessoryStatus)
                        {
                            if (accValue.Value > 0 && accValue.ItemId == accessoriesWithValueGreaterThanZero.ItemId)
                            {
                                var socketValue = (int)(accValue.Value * applyAtt);
