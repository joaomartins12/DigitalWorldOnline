...
namespace DigitalWorldOnline.GameHost
{
    public sealed partial class MapServer
    {
                ...
                case MobActionEnum.Respawn:
                    {
                        mob.Reset();
                        mob.ResetLocation();
                        if (mob.RespawnInterval > 3599) CallDiscordWarnings($"[SPAWN] {mob.Name} nasceu em {map.Name} CH{map.Channel}.", "81ffcf", "1279317585652879391", "1280691378887725158");
                    }
                    break;
                ...
                case MobActionEnum.Reward:
                    {
                        ...
                        if (mob.RespawnInterval > 3599)
                        {
                            ...
                            CallDiscordWarnings($"[DEAD] {mob.Name} foi morto em {map.Name} CH{map.Channel}. \nNascerá novamente as {formattedTime} (UTC-03), em {(int)(mob.RespawnInterval / 3600)}hrs!", "ff0000", "1279317585652879391", "1280691378887725158");
                        }
                        ItemsReward(map, mob);
        ...
        private void SkillExpReward(GameMap map, GameClient? targetClient)
        {
            ...
            if (targetClient.Partner.CurrentEvolution.SkillMastery < 30)    // Skill Mastery bloqueia exp no 15
            {
                if (targetClient.Partner.CurrentEvolution.SkillExperience >= ExpNeed)
                {
                    targetClient.Partner.ReceiveSkillPoint();   // Increase skill point by 8
        ...
        private void ItemDropReward(GameMap map, MobConfigModel mob, GameClient? targetClient)
        {
            ...
            foreach (var itemDrop in _configs.GlobalDrops.ToList())
            {
                if (DateTime.Now >= itemDrop.StartTime && DateTime.Now <= itemDrop.EndTime)
                {
                    if (itemDrop.Map == 0 || itemDrop.Map == targetClient.Tamer.Location.MapId)
                    {
                        ...
                        var newItem = new ItemDropConfigModel
                        {
                            ...
                            MinAmount = itemDrop.MinDrop,
                            MaxAmount = itemDrop.MaxDrop,
