...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class DigimonTranscendenceExpPacketProcessor : IGamePacketProcessor
    {
        ...
        {
            // Limpa a lista antes de processar um novo pacote de transcendência
            ...
            foreach (var targetSlot in _transcendSlots)
            {
                ...
                Console.WriteLine($"[Process] Removendo Digimon no slot {targetSlot}. ID do Digimon: {targetPartner.Digimon?.Id}");
                ...
                if (targetPartner.Digimon != null)
                {
                    if (!targetPartner.Digimon.IsRaremonType)
                    {
                        if (inputType == AcademyInputType.Low)
                        {
                            ...
                            //if (Chance >= Success)
                            if (Chance >= Success || Chance >= 50)
                            {
                                Console.WriteLine($"[Process] CASO 1");
                                successRate = 1;
                                chargeExp += BonusExp;
                        ...
                        else if (inputType == AcademyInputType.High)
                        {
                            ...
                            if (Chance >= Success || Chance >= 50)
                            {
                                Console.WriteLine($"[Process] CASO 2");
                                successRate = 1;
                                chargeExp += BonusExp;
                                digivicePartner.UpdateTranscendenceExp((long)(BonusExp + digivicePartner.TranscendenceExperience));
                            }
                    ...
                    else if (targetPartner.Digimon.IsRaremonType)
                    {
                        if (inputType == AcademyInputType.Low)
                        {
                            ...
                            if (targetPartner.Digimon.HatchGrade == DigimonHatchGradeEnum.High)
                            {
                                ...
                                if (Chance >= Success || Chance >= 50)
                                {
                                    Console.WriteLine($"[Process] CASO 4");
                                    successRate = 1;
                                    chargeExp += BonusExp;
                                    digivicePartner.UpdateTranscendenceExp((long)(BonusExp + digivicePartner.TranscendenceExperience));
                                }
                            ...
                            if (targetPartner.Digimon.HatchGrade == DigimonHatchGradeEnum.Perfect)
                            {
                                ...
                                if (Chance >= Success || Chance >= 50)
                                {
                                    Console.WriteLine($"[Process] CASO 5");
                                    successRate = 1;
                                    chargeExp += BonusExp;
                                    digivicePartner.UpdateTranscendenceExp((long)(BonusExp + digivicePartner.TranscendenceExperience));
                                }
                        ...
                        else if (inputType == AcademyInputType.High)
                        {
                            if (targetPartner.Digimon.HatchGrade == DigimonHatchGradeEnum.Default)
                            {
                                ...
                                if (Chance >= Success)
                                {
                                    Console.WriteLine($"[Process] CASO 6");
                                    successRate = 1;
                                    chargeExp += BonusExp;
                                    digivicePartner.UpdateTranscendenceExp((long)(BonusExp + digivicePartner.TranscendenceExperience));
                                }
                            ...
                            if (targetPartner.Digimon.HatchGrade == DigimonHatchGradeEnum.High)
                            {
                                ...
                                if (Chance >= Success || Chance >= 50)
                                {
                                    Console.WriteLine($"[Process] CASO 7");
                                    successRate = 1;
                                    chargeExp += BonusExp;
                                    digivicePartner.UpdateTranscendenceExp((long)(BonusExp + digivicePartner.TranscendenceExperience));
                                }
                            ...
                            if (targetPartner.Digimon.HatchGrade == DigimonHatchGradeEnum.Perfect)
                            {
                                ...
                                if (Chance >= Success || Chance >= 50)
                                {
                                    Console.WriteLine($"[Process] CASO 8");
                                    successRate = 1;
                                    chargeExp += BonusExp;
                                    digivicePartner.UpdateTranscendenceExp((long)(BonusExp + digivicePartner.TranscendenceExperience));
                    ...
                    // Verifica se o slot está vazio ou se o Digimon já foi removido
                    if (targetPartner == null || targetPartner.Digimon == null)
                    {
                        Console.WriteLine($"[Process] Slot {targetSlot} já está vazio ou Digimon removido.");
                    ...
                    else
                    {
                        ...
                        Console.WriteLine($"[Process] Digimon removido do slot {targetSlot}.");
            ...
            //Console.WriteLine($"[Process] Item removido ou reduzido no slot {itemSlot}. Quantidade: {amount}");

            await _sender.Send(new UpdateItemsCommand(client.Tamer.Inventory));
            //Console.WriteLine($"[Process] Comando de atualização do inventário enviado.");

            long Envio = digivicePartner.TranscendenceExperience;
            if (Envio > 140000) Envio = 140000;
            client.Send(new DigimonTranscendenceReceiveExpPacket(inputType, (byte)targetAcademySlot, digimonCount, _transcendSlots, itemSlot, targetItem, (short)successRate, chargeExp, (long)Envio));
            // Console.WriteLine($"[Process] Pacote de recebimento de experiência enviado.");

            client.Send(new LoadInventoryPacket(client.Tamer.Inventory, InventoryTypeEnum.Inventory).Serialize());
            //Console.WriteLine($"[Process] Pacote de carregamento do inventário enviado.");

            await _sender.Send(new UpdateDigimonExperienceCommand(digivicePartner));
            //Console.WriteLine($"[Process] Comando de atualização da experiência do Digimon enviado.");
