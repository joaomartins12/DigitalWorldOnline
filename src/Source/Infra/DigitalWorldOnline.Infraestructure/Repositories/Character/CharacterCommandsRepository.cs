...
namespace DigitalWorldOnline.Infraestructure.Repositories.Character
{
    public class CharacterCommandsRepository : ICharacterCommandsRepository
    {
        ...
        public async Task<AccountBlockDTO> AddBanAsync(AccountBlockModel ban)
        {
            var dto = _mapper.Map<AccountBlockDTO>(ban);
        ...
        public async Task<CharacterFriendDTO> ChangeFriendMemoAsync(long Id, string NewMemo)
        {
            var dto = await _context.CharacterFriends.AsNoTracking().FirstOrDefaultAsync(x => x.Id == Id);

            if (dto != null)
            {
                dto.Annotation = NewMemo;
