...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class ItemSplitPacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            {
                await _mapServer.CallDiscord($"try DUPPING {amountToSplit}x {sourceItemCheck.ItemInfo.Name}, but he has {sourceItemCheck.Amount}x on slot {originSlot} to {destinationSlot}!!", client, "e06666", "DISCONNECTED", "1280704020469514291");
                client.Disconnect();
                return;
            }
